...
class GcnNet(nn.Module):
    def __init__(self, input_dim=1433):
        ...
        self.gcn1 = GraphConvolution(input_dim, 64)  # (N, 1433) -> (N, 64)
        self.gcn2 = GraphConvolution(64, 32)  # (N, 64) -> (N, 32)
        self.gcn3 = GraphConvolution(32, 16)  # (N, 32) -> (N, 16)
        self.gcn4 = GraphConvolution(16, 7)  # (N, 16) -> (N, 7)

    def forward(self, adjacency, feature):
        h1 = F.relu(self.gcn1(adjacency, feature))  # (N, 1433) -> (N, 64)
        h2 = F.relu(self.gcn2(adjacency, h1))  # (N, 64) -> (N, 32)
        h3 = F.relu(self.gcn3(adjacency, h2))  # (N, 32) -> (N, 16)
        logits = self.gcn4(adjacency, h3)  # (N, 16) -> (N, 7)
        return logits
# class GcnNet(nn.Module):
#     def __init__(self, input_dim=1433):
#             super(GcnNet, self).__init__()
#             self.gcn_layers = nn.ModuleList([GraphConvolution(input_dim, 64)])  # 第一层，输入维度到64维
#
#             for _ in range(9):  # 添加九层中间层
#                 self.gcn_layers.append(GraphConvolution(64, 64))
#
#             self.gcn_layers.append(GraphConvolution(64, 7))  # 最后一层，64维到7维
#
#     def forward(self, adjacency, feature):
#             h = feature  # 初始特征
#             for layer in self.gcn_layers:
#                 h = F.relu(layer(adjacency, h))  # 应用ReLU激活函数
#             logits = h  # 最后一层输出
#             return logits
    """
  输入特征维度为1433 经过两个卷积层降到7维
  h为隐藏层的输出 将h传递给第二个卷积
  接受图的邻接矩阵和节点特征作为输入，通过两个图卷积层进行信息传播和特征抽取
  并输出每个节点属于不同类别的分数
    """
...
def plot_loss_with_acc(loss_history, val_acc_history):
    fig1 = plt.figure(1)
    # 坐标系ax1画曲线1
    ax1 = fig1.add_subplot(111)  # 指的是将plot界面分成1行1列，此子图占据从左到右从上到下的1位置
    ax1.plot(range(len(loss_history)), loss_history,
             c=np.array([255, 71, 90]) / 255.)  # c为颜色
    plt.ylabel('Loss')

    # 坐标系ax2画曲线2
    ax2 = fig1.add_subplot(111, sharex=ax1, frameon=False)  # 其本质就是添加坐标系，设置共享ax1的x轴，ax2背景透明
...
print(f'Test accuarcy:  {test_acc.item():.4f}')
